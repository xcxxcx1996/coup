    ...
    return (
        <Dialog onClose={handleClose} open={open}>
            ...
            >
                <AbilityBtn
                    text="女王（防刺杀）"
                    onClick={() => nakamaClient.denyKill()}
                />
                <AbilityBtn
                    text="男爵（收3金币）"
                    onClick={() => nakamaClient.drawThreeCoins()}
                />
                <AbilityBtn
                    text="男爵（阻止收2金币）"
                    onClick={() => nakamaClient.denyMoney()}
                />
                <AbilityBtn text="大使（换牌）" onClick={null} />
                <AbilityBtn
                    text="大使（阻止偷金币）"
                    onClick={() => nakamaClient.denySteal()}
                />
                <MenuButton
                    text={"队长（偷2金币）"}
                    items={users.map((u) => ({
                        text: u.name,
                        onClick: () => nakamaClient.stealCoins(u.id),
                    }))}
                    btnWidth={250}
                    menuItemWidth={250}
                />
                ...
                <MenuButton
                    ...
                    items={users.map((u) => ({
                        text: u.name,
                        onClick: () => nakamaClient.assassin(u.id),
                    }))}
                    btnWidth={250}
                    menuItemWidth={250}
                />
